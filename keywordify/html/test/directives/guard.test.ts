...
import { Directive, directive, type PartInfo } from '../../src/directive.js';
...
suite('guard', () => {
  ...
  function renderGuarded(value: unknown, f: () => unknown) {
    render(html`<div>${guard(value as unknown[], f)}</div>`, container);
  }
