...
import type { DirectiveResult } from '../../src/directive.js';
...
type GetRenderAs<D extends DirectiveResult> =
  D extends DirectiveResult<infer C>
    ? C extends {
        new (
          ...args: never[]
        ): { [K.render](...args: never[]): infer RenderAs };
      }
      ? RenderAs
      : unknown
    : unknown;
...
if (false as boolean) {
  ...
  () => {
    ...
    // @ts-expect-error
    vRendersAs3 satisfies Promise<unknown>;
    ...
    // @ts-expect-error
    vRendersAs4 satisfies Promise<unknown>;
  };
}
