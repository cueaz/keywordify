...
import { unsafeSVG } from '../../src/directives/unsafe-svg.js';
...
suite('unsafeSVG', () => {
  ...
  test('throws on non-string values', () => {
    const value = ['aaa'];
    const t = () => html`<div>${unsafeSVG(value as unknown as string)}</div>`;
  ...
  test('renders after other values', () => {
    ...
    const primitive = 'aaa';
    const t = (content: unknown) => html`<svg>${content}</svg>`;
