...
suite('ref', () => {
  ...
  test('sets a ref on a Ref object', () => {
    ...
    assert.equal(divRef[K.value], div);
  });
  ...
  test('sets a ref when Ref object changes', () => {
    ...
    assert.equal(divRef1[K.value], div1);

    go(divRef2);
    const div2 = container.firstElementChild;
    assert.equal(divRef1[K.value], undefined);
    assert.equal(divRef2[K.value], div2);
  });
  ...
  test('only sets a ref when element changes', () => {
    ...
    Object.defineProperty(elRef, K.value, {
    ...
    assert.equal(elRef[K.value], queriedEl);
  ...
  test('two refs', () => {
    ...
    const div = container.firstElementChild;
    assert.equal(divRef1[K.value], div);
    assert.equal(divRef2[K.value], div);
  });
  ...
  test('refs are always set in tree order', () => {
    ...
    go();
    assert.equal(elRef[K.value]!.id, 'last');
    go();
    assert.equal(elRef[K.value]!.id, 'last');
  });
  ...
  test('Ref passed to ref directive changes', () => {
    ...
    go(true);
    assert.equal(aRef[K.value]?.tagName, 'DIV');
    assert.equal(bRef[K.value], undefined);
    go(false);
    assert.equal(aRef[K.value], undefined);
    assert.equal(bRef[K.value]?.tagName, 'DIV');
    go(true);
    assert.equal(aRef[K.value]?.tagName, 'DIV');
    assert.equal(bRef[K.value], undefined);
  });
  ...
  test('disconnect gracefully with an undefined ref', () => {
