...
suite('until directive', () => {
  ...
  test('renders changing defaultContent', async () => {
    const t = (d: unknown) => html`<div>${until(deferred.promise, d)}</div>`;
  ...
  test('renders a Promise to a property', async () => {
    ...
    assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
    assert.equal((div as unknown as { test?: unknown }).test, undefined);
    await promise;
    assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
    assert.equal((div as unknown as { test?: unknown }).test, 'foo');
  });
  ...
  test('renders racing Promises across renders correctly', async () => {
    const deferred1 = new Deferred<unknown>();
    const deferred2 = new Deferred<unknown>();

    const t = (promise: unknown) => html`<div>${until(promise)}</div>`;
  ...
  test('renders Promises with changing priorities', async () => {
    ...
    const t = (p1: unknown, p2: unknown) => html`<div>${until(p1, p2)}</div>`;
  ...
  test('renders a literal in a PropertyPart', () => {
    ...
    assert.equal(
      (
        container.querySelector('div')! as unknown as {
          someProp?: unknown;
        }
      ).someProp,
      'a',
  ...
  test('renders later arguments until earlier promises resolve', async () => {
    let resolvePromise: (arg: unknown) => void;
  ...
  test(
    ...
    async () => {
      let resolvePromiseA: (arg: unknown) => void;
      const promiseA = new Promise((resolve, _reject) => {
        resolvePromiseA = resolve;
      });

      let resolvePromiseB: (arg: unknown) => void;
