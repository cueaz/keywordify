...
import { AsyncDirective } from '../src/async-directive.js';
import {
  Directive,
...
import { isCompiledTemplateResult } from '../src/directive-helpers.js';
import { createRef, ref } from '../src/directives/ref.js';
...
import { literal, unsafeStatic } from '../src/static.js';
import {
  stripDynamicFallbackTags,
  stripExpressionComments,
  stripExpressionMarkers,
} from './test-utils/strip-markers.js';

const AttributePart = _$LH[K.AttributePart];
...
class FireEventDirective extends Directive {
  ...
  override [K.update](part: AttributePart) {
    part[K.element].dispatchEvent(
...
suite('lit-html', () => {
  ...
  suite('marker insertion', () => {
    ...
    test('"dynamic" tag name', () => {
      ...
      if (DEV_MODE) {
        ...
        render(template, container);
        assert.equal(
          stripExpressionMarkers(stripDynamicFallbackTags(container.innerHTML)),
          '<></>',
        );
      }
    });
    ...
    test('comment', () => {
      ...
      // compiled templates.
      assert.equal(
        container.innerHTML.replace(/z\ue001[\u28f1-\u28ff]{30}\ue001/g, ''),
        '<!----><!---->',
      );
    });
  ...
  suite('text', () => {
    ...
    [nothing, undefined, null, ''].forEach((value: unknown) => {
      test(`renders '${
        ...
        const template = (i: unknown) => html`<div>${i}</div>`;
  ...
  suite('attributes', () => {
    ...
    test.skip('renders a Symbol in an array to an attribute', () => {
      render(html`<div foo=${[Symbol('A')] as unknown}></div>`, container);
      assert.include(container.querySelector('div')!.getAttribute('foo')!, '');
    });
    ...
    test('does not call a function bound to an attribute', () => {
      ...
      render(html`<div foo=${f as unknown}></div>`, container);
      const div = container.querySelector('div')!;
      assert.isTrue(div.hasAttribute('foo'));
    });

    test('renders an array to an attribute', () => {
      render(html`<div foo=${['1', '2', '3'] as unknown}></div>`, container);
    ...
    test('renders undefined in attributes', () => {
      render(html`<div attribute="${undefined as unknown}"></div>`, container);
      assertContent('<div attribute=""></div>');
    });

    test('renders null in attributes', () => {
      render(html`<div attribute="${null as unknown}"></div>`, container);
      assertContent('<div attribute=""></div>');
    });
    ...
    test('initial render of noChange in fully-controlled attribute', () => {
      render(html`<div attribute="${noChange as unknown}"></div>`, container);
      assertContent('<div></div>');
    });
    ...
    test('nothing sentinel removes an attribute', () => {
      const go = (v: unknown) => html`<div a=${v}></div>`;
    ...
    test('interpolated nothing sentinel removes an attribute', () => {
      const go = (v: unknown) => html`<div a="A${v}"></div>`;
      render(go('a'), container);
  ...
  suite('properties', () => {
    test('sets properties', () => {
      ...
      const div = container.querySelector('div')!;
      assert.strictEqual((div as unknown as { foo?: unknown }).foo, 123);
      assert.strictEqual((div as unknown as { Bar?: unknown }).Bar, 456);
    });
    ...
    test('nothing becomes undefined', () => {
      ...
      go(1);
      const div = container.querySelector('div')!;
      assert.strictEqual((div as unknown as { foo?: unknown }).foo, 1);

      go(nothing);
      assert.strictEqual((div as unknown as { foo?: unknown }).foo, undefined);
    });
    ...
    test('null sets null', () => {
      ...
      go(null);
      const div = container.querySelector('div')!;
      assert.strictEqual((div as unknown as { foo?: unknown }).foo, null);
    });

    test('null in multiple part sets empty string', () => {
      const go = (v1: unknown, v2: unknown) =>
        render(html`<div .foo="${v1}${v2}"></div>`, container);

      go('hi', null);
      const div = container.querySelector('div')!;
      assert.strictEqual((div as unknown as { foo?: unknown }).foo, 'hi');
    });
  ...
  suite('events', () => {
    ...
    test('adds event listener functions, calls with right this value', () => {
      ...
      const listener = function (this: unknown, e: Event) {
        event = e;
        thisValue = this;
      };
    ...
    test('removes event listeners', () => {
      let target: unknown;
      let listener: unknown = (e: Event) => (target = e.target);
      const t = () => html`<div @click=${listener}></div>`;
      render(t(), container);
      const div = container.querySelector('div')!;
      div.click();
  ...
  suite('updates', () => {
    ...
    test('updates an element', () => {
      let child: Node | undefined = document.createElement('p');
  ...
  suite('compiled', () => {
    ...
    test('element expression', () => {
      ...
      assert.strictEqual(r[K.value], div);
    });

    test(`throw if unbranded`, () => {
      const _$lit_template_1: CompiledTemplate = {
        [K.h]: ['<div><!----></div>'] as unknown as TemplateStringsArray,
        [K.parts]: [{ [K.type]: 2, [K.index]: 1 }],
      };
      const result = {
        // This property needs to remain unminified.
  ...
  suite('directives', () => {
    ...
    test('PartInfo includes metadata for directive in ChildPart', () => {
      ...
      assert.equal(partInfo![K.type], PartType[K.CHILD]);
    });
    ...
    suite('ChildPart invariants for parentNode, startNode, endNode', () => {
      ...
      class CheckNodePropertiesBehavior extends Directive {
        ...
        [K.render](_parentId?: string, _done?: (err?: unknown) => void) {
          return nothing;
        }
        ...
        override [K.update](
          ...
          try {
            const parentNode = part[K.parentNode];
            const startNode = part[K.startNode];
            const endNode = part[K.endNode];
      ...
      test(`when the parentNode is null`, async () => {
        ...
        assert.equal(currentDirective.part![K.parentNode], undefined);
      });
    ...
    test('directives can update', () => {
      ...
      assert.equal(receivedPart![K.type], PartType[K.CHILD]);
    ...
    test('PartInfo includes metadata for directive in AttributeParts', () => {
      ...
      if (partInfo![K.type] !== PartType[K.ATTRIBUTE]) {
        throw new Error('Expected attribute PartInfo');
      }
      assert.equal(partInfo![K.tagName], 'DIV');
      assert.equal(partInfo![K.name], 'title');
      assert.deepEqual(partInfo![K.strings], ['a ', ' b']);
    });
    ...
    test('renders directives on PropertyParts', () => {
      ...
      assert.strictEqual(
        (container.firstElementChild as unknown as { foo?: unknown }).foo,
        'A:1',
      );
    });
    ...
    test('PartInfo includes metadata for directive in PropertyParts', () => {
      ...
      if (partInfo![K.type] !== PartType[K.PROPERTY]) {
    ...
    test('renders directives on EventParts', () => {
      const handle = directive(
        class extends Directive {
          count = 0;
          [K.render](value: string) {
            return (e: Event) => {
              (e.target as unknown as { __clicked?: string }).__clicked =
                `${value}:${++this.count}`;
            };
          }
        },
      ...
      assert.strictEqual(
        (container.firstElementChild as unknown as { __clicked?: string })
          .__clicked,
        'A:1',
      );
      (container.firstElementChild as HTMLDivElement).click();
    ...
    test('directives have access to renderOptions', () => {
      ...
      class HostDirective extends Directive {
        ...
        override [K.update](part: Part, props: [v: string]) {
          this.host ??= part[K.options]?.[K.host] as HTMLInputElement;
    ...
    suite('nested directives', () => {
      const aNothingDirective = directive(
        class extends Directive {
          [K.render](bool: boolean, v: unknown) {
  ...
  suite('async directives', () => {
    class ADirective extends AsyncDirective {
      ...
      [K.render](_promise: Promise<unknown>) {
        return 'initial';
      }
      override [K.update](
        _part: Part,
        [promise]: Parameters<this[typeof K.render]>,
      ) {
        aDirectiveInst = this;
        if (promise !== this.promise) {
          ...
          promise.then((value) =>
            (() => {
              this.value = value;
              return this[K.setValue](value);
            })(),
          );
        }
        return this.value ?? this[K.render](promise);
      }
    }
    ...
    test('async directives change to disconnected in ChildPart', async () => {
      ...
      assert.isTrue(aDirectiveInst[K.isConnected]);
      part[K.setConnected](false);
      assertContent(`<div>initial</div>`);
      await promise;
      assert.isFalse(aDirectiveInst[K.isConnected]);
      assertContent(`<div>resolved1</div>`);
    ...
    const disconnectingDirective = directive(
      class extends AsyncDirective {
        ...
        [K.render](log: Array<string>, id = '', value?: unknown, bool = true) {
          this.log = log;
          this.id = id;
    ...
    const passthroughDirective = directive(
      class extends Directive {
        [K.render](value: unknown, bool = true) {
          return bool ? value : nothing;
        }
      },
    );
  ...
  //     );
  //     const div = container.querySelector('div');
  //     assert.equal((div as unknown as { foo?: unknown }).foo, 'bar');
  //   });
  // });
  ...
  securityHooksSuiteFunction('enhanced security hooks', () => {
    ...
    const sanitizerCalls: Array<{
      name: string;
      type: typeof K.property | typeof K.attribute | 'text';
      nodeName: string;
    ...
    beforeEach(() => {
      render[K.setSanitizer]?.(testSanitizerFactory);
    });
    afterEach(() => {
    ...
    test('sanitizes properties', () => {
      ...
      assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
      assert.equal(
        (container.querySelector('div')! as unknown as { foo?: unknown }).foo,
        'safeString',
      );
  ...
  warningsSuiteFunction('warnings', () => {
    let originalWarn: (...data: unknown[]) => void;
